...
-- Ordered list for dropdown so items appear in consistent order
local GLOW_STYLE_ORDER = {1, 2, 3}
local GLOW_STYLE_NAMES = {
    [1] = "Action Button",
    [2] = "Pixel",
    [3] = "Proc Glow",
}
...
function OPTIONS_FRAME:add_glow_settings()
    ...
    local function GetCurrentGlowStyleName()
        return GLOW_STYLE_NAMES[SWITCHES.PROC_GLOW_STYLE] or GLOW_STYLE_NAMES[1]
    end

    local function GlowStyle_OnClick(btn)
        SWITCHES.PROC_GLOW_STYLE = btn.value
        UIDropDownMenu_SetSelectedValue(glowStyleDropDown, btn.value)
        UIDropDownMenu_SetText(glowStyleDropDown, GLOW_STYLE_NAMES[btn.value])
        CloseDropDownMenus()
        refresh_all_procs()
    end

    local function GlowStyle_Init(self, level)
        local currentStyle = SWITCHES.PROC_GLOW_STYLE or 1
        for _, styleValue in ipairs(GLOW_STYLE_ORDER) do
            local info = UIDropDownMenu_CreateInfo()
            info.text = GLOW_STYLE_NAMES[styleValue]
            info.value = styleValue
            info.func = GlowStyle_OnClick
            info.checked = (styleValue == currentStyle)
            UIDropDownMenu_AddButton(info, level or 1)
        end
    end

    UIDropDownMenu_Initialize(glowStyleDropDown, GlowStyle_Init)
    UIDropDownMenu_SetSelectedValue(glowStyleDropDown, SWITCHES.PROC_GLOW_STYLE or 1)
    UIDropDownMenu_SetText(glowStyleDropDown, GetCurrentGlowStyleName())
    ...
    config_frame:SetScript("OnShow", function()
        -- Re-initialize dropdown and set current selection
        UIDropDownMenu_Initialize(glowStyleDropDown, GlowStyle_Init)
        UIDropDownMenu_SetSelectedValue(glowStyleDropDown, SWITCHES.PROC_GLOW_STYLE or 1)
        UIDropDownMenu_SetText(glowStyleDropDown, GetCurrentGlowStyleName())
...
local function FontDropDown_OnClick(self, arg1)
    ...
    if fontDropDown._previewText then
        local fontFile = SWITCHES.FONT_FILE
        if fontFile and fontFile ~= "" then
            pcall(function()
                fontDropDown._previewText:SetFont(fontFile, 14, "OUTLINE")
            end)
        end
    end
end
...
local function FontDropDown_Menu(frame, level, menuList)
    ...
    if level == 1 then
        ...
        -- Set current font name in dropdown text
        local current_found = false
        for font_name, font_path in pairs(font_table) do
            if font_path == SWITCHES.FONT_FILE then
                UIDropDownMenu_SetText(fontDropDown, font_name)
                current_found = true
                break
            end
        end
...
function OPTIONS_FRAME:add_font_settings()
    if not LSM then return end

    local config_frame = CreateFrame("Frame", nil, self)
    config_frame.name = "Fonts"
    config_frame.parent = self.main_options.name

    local title = config_frame:CreateFontString(nil, nil, "GameFontNormalLarge")
    title:SetPoint("TOPLEFT", MARGIN, -MARGIN)
    title:SetText("Font Settings")

    local font_label = config_frame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    font_label:SetPoint("TOPLEFT", MARGIN, -MARGIN * 3.5)
    font_label:SetText("Font:")

    fontDropDown = CreateFrame("Frame", ADDON_NAME.."FontDropDown", config_frame, "UIDropDownMenuTemplate")
    fontDropDown:SetPoint("TOPLEFT", MARGIN * 3, -MARGIN * 3.2)
    UIDropDownMenu_SetWidth(fontDropDown, 200)
    UIDropDownMenu_Initialize(fontDropDown, FontDropDown_Menu)

    local preview_label = config_frame:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    preview_label:SetPoint("TOPLEFT", MARGIN, -MARGIN * 6)
    preview_label:SetText("Font preview:")

    local preview_text = config_frame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    preview_text:SetPoint("TOPLEFT", MARGIN + 10, -MARGIN * 7.2)
    preview_text:SetText("0123456789  AaBbCcDd")

    -- Store reference so FontDropDown_OnClick can update it
    fontDropDown._previewText = preview_text

    local info_text = config_frame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    info_text:SetPoint("TOPLEFT", MARGIN, -MARGIN * 9)
    info_text:SetText("Fonts are provided by LibSharedMedia-3.0.\nInstall font addons (e.g. SharedMedia) for more options.")
    ...
    config_frame:SetScript("OnShow", function()
        ...
        local fontFile = SWITCHES.FONT_FILE
        if fontFile and fontFile ~= "" then
            pcall(function() preview_text:SetFont(fontFile, 14, "OUTLINE") end)
        end
    end)
